...
{
    [Authorize]
    public class DinhMucFilletController : Controller
    {
        private readonly IHttpClientFactory _httpClientFactory;
        public DinhMucFilletController(IHttpClientFactory httpClientFactory)
        {
            _httpClientFactory = httpClientFactory;
        }
        [CustomAuthorize(Fu = "Danh Mục / Fillet / Định Mức", Func = "Xem Fillet / Định Mức")]
        public IActionResult Index()
        {
            var rl = Middlewares.AuthenticationHelpers.CheckAut(HttpContext, "DinhMucFilletView");
            ...
            ViewBag.TitlePage = "Định Mức Fillet";
            return View("~/Views/DanhMuc/Fillet/DinhMucFilletView.cshtml");
        }
        public async Task<IEnumerable<object>> GetAllsFullField()
        {
            ...
            if (dataSource == null)
            {
                var apiUrl = $"{AppViewModels.AppViewModel.Instance.ApiHostUrl}/api/DinhMucFillets/GetAllsFullField";
        ...
        public async Task<IEnumerable<DinhMucFillet>> GetAlls()
        {
            IEnumerable<DinhMucFillet> dataSource = ViewBag.dataSource;
            if (dataSource == null)
            {
                var apiUrl = $"{AppViewModels.AppViewModel.Instance.ApiHostUrl}/api/DinhMucFillets/GetAlls";
                using var helper = new Middlewares.MethodRESTFulAPIHelpers(_httpClientFactory);
                ViewBag.dataSource = await helper.GetAsync<IEnumerable<DinhMucFillet>>(HttpContext, apiUrl);
                dataSource = ViewBag.dataSource;
            }
            return dataSource;
        }
        ...
        [CustomAuthorize(Fu = "Danh Mục / Fillet / Định Mức", Func = "Thêm Fillet / Định Mức")]
        ...
        {
            var apiUrl = $"{AppViewModels.AppViewModel.Instance.ApiHostUrl}/api/DinhMucFillets/Insert";
            try
            {
        ...
        public async Task<IActionResult> GetsByMa(string stt, string ngay, string gio, string maLo, string maLoaiCa, string maMau, string maSize, string maThanhPham, string maXuong, string caTra)
        {
            ...
            if (dataSource == null)
            {
                var apiUrl = $"{AppViewModels.AppViewModel.Instance.ApiHostUrl}/api/DinhMucFillets/GetsByMa/{stt.ToString()}/{ngay.ToString()}/{gio.ToString()}/{maLo}/{maLoaiCa}/{maMau}/{maSize}/{maThanhPham}/{maXuong}/{caTra.ToString()}";
                using var helper = new Middlewares.MethodRESTFulAPIHelpers(_httpClientFactory);
                var item = await helper.GetAsync<DinhMucFillet>(HttpContext, apiUrl);
                if (item != null)
                {
                    return Json(new
                    {
        ...
        [CustomAuthorize(Fu = "Danh Mục / Fillet / Định Mức", Func = "Sửa Fillet / Định Mức")]
        ...
        {
            var apiUrl = $"{AppViewModels.AppViewModel.Instance.ApiHostUrl}/api/DinhMucFillets/Update/{stt}/{ngay}/{gio}/{maLo}/{maLoaiCa}/{maMau}/{maSize}/{maThanhPham}/{maXuong}/{caTra}";
            try
            {
        ...
        [CustomAuthorize(Fu = "Danh Mục / Fillet / Định Mức", Func = "Xoá Fillet / Định Mức")]
        public async Task<IActionResult> DoDelete(string stt, string ngay, string gio, string maLo, string maLoaiCa, string maMau, string maSize, string maThanhPham, string maXuong, string caTra)
        {
            try
            {
                var apiUrl = $"{AppViewModels.AppViewModel.Instance.ApiHostUrl}/api/DinhMucFillets/Delete/{stt}/{ngay}/{gio}/{maLo}/{maLoaiCa}/{maMau}/{maSize}/{maThanhPham}/{maXuong}/{caTra}";
                using var helper = new Middlewares.MethodRESTFulAPIHelpers(_httpClientFactory);
