...
namespace PMS.Controllers.DanhMuc.XepKhuon.QuanLyPhuGia
{
    ...
    public class DonViTinhController : Controller
    {
        ...
        public DonViTinhController(IHttpClientFactory httpClientFactory)
        {
        ...
        {
            var rl = Middlewares.AuthenticationHelpers.CheckAut(HttpContext, "DonViTinhView");
            ...
            return View("~/Views/DanhMuc/XepKhuon/QuanLyPhuGia/DonViTinhView.cshtml");
        }
        public async Task<IEnumerable<PD_DonViTinh>> GetAlls()
        {
            IEnumerable<PD_DonViTinh> dataSource = ViewBag.dataSource;
            if (dataSource == null)
            {
                var apiUrl = $"{AppViewModels.AppViewModel.Instance.ApiHostUrl}/api/PD_DonViTinh/GetAlls";
                using var helper = new Middlewares.MethodRESTFulAPIHelpers(_httpClientFactory);
                ViewBag.dataSource = await helper.GetAsync<IEnumerable<PD_DonViTinh>>(HttpContext, apiUrl);
        ...
        {
            var apiUrl = $"{AppViewModels.AppViewModel.Instance.ApiHostUrl}/api/PD_DonViTinh/Insert";
            try
            {
        ...
        public async Task<IActionResult> GetsByMa(string ma)
        {
            ...
            if (dataSource == null)
            {
                var apiUrl = $"{AppViewModels.AppViewModel.Instance.ApiHostUrl}/api/PD_DonViTinh/GetsByMa/{ma}";
                using var helper = new Middlewares.MethodRESTFulAPIHelpers(_httpClientFactory);
                var item = await helper.GetAsync<PD_DonViTinh>(HttpContext, apiUrl);
                if (item != null)
                {
                    return Json(new
                    {
        ...
        public async Task<IActionResult> DoDelete(string ma)
        {
            try
            {
                var apiUrl = $"{AppViewModels.AppViewModel.Instance.ApiHostUrl}/api/PD_DonViTinh/Delete/{ma}";
                using var helper = new Middlewares.MethodRESTFulAPIHelpers(_httpClientFactory);
