...
{
    [Authorize]
    public class ThanhPhamChinhXepKhuonController : Controller
    {
        private readonly IHttpClientFactory _httpClientFactory;
        public ThanhPhamChinhXepKhuonController(IHttpClientFactory httpClientFactory)
        {
            _httpClientFactory = httpClientFactory;
        }
        [CustomAuthorize(Fu = "Danh Mục / Xếp Khuôn / Thành Phẩm Chính", Func = "Xem Xếp Khuôn / Thành Phẩm Chính")]
        public IActionResult Index()
        {
            var rl = Middlewares.AuthenticationHelpers.CheckAut(HttpContext, "ThanhPhamChinhXepKhuonView");
            ...
            return View("~/Views/DanhMuc/XepKhuon/Chinh/ThanhPhamChinhXepKhuonView.cshtml");
        }
        public async Task<IEnumerable<MaThanhPhamChinhXepKhuon>> GetAlls()
        {
            IEnumerable<MaThanhPhamChinhXepKhuon> dataSource = ViewBag.dataSource;
            if (dataSource == null)
            {
                var apiUrl = $"{AppViewModels.AppViewModel.Instance.ApiHostUrl}/api/MaThanhPhamChinhXepKhuons/GetAlls";
                using var helper = new Middlewares.MethodRESTFulAPIHelpers(_httpClientFactory);
                ViewBag.dataSource = await helper.GetAsync<IEnumerable<MaThanhPhamChinhXepKhuon>>(HttpContext, apiUrl);
        ...
        public async Task<IActionResult> DoInsert(string ma, string ten, double min, double max, string bravoId,bool suDung, int type, decimal thamSoTangTrong,decimal dinhMucTangTrong, bool isKhongThuoc)
        {
            ...
            try
            {
                ...
                var model = new MaThanhPhamChinhXepKhuon
                {
                    Ma = ma,
                    Ten = ten,
                    SuDung = suDung,
        ...
        public async Task<IActionResult> GetsByMa(string ma)
        {
            ...
            if (dataSource == null)
            {
                var apiUrl = $"{AppViewModels.AppViewModel.Instance.ApiHostUrl}/api/MaThanhPhamChinhXepKhuons/GetsByMa/{ma}";
                using var helper = new Middlewares.MethodRESTFulAPIHelpers(_httpClientFactory);
                var item = await helper.GetAsync<MaThanhPhamChinhXepKhuon>(HttpContext, apiUrl);
                if (item != null)
                {
                    return Json(new
                    {
        ...
        [CustomAuthorize(Fu = "Danh Mục / Xếp Khuôn / Thành Phẩm Chính", Func = "Xóa Xếp Khuôn / Thành Phẩm Chính")]
        public async Task<IActionResult> DoDelete(string ma)
        {
            try
            {
                var apiUrl = $"{AppViewModels.AppViewModel.Instance.ApiHostUrl}/api/MaThanhPhamChinhXepKhuons/Delete/{ma}";
                using var helper = new Middlewares.MethodRESTFulAPIHelpers(_httpClientFactory);
