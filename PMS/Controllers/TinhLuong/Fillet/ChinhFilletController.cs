...
namespace PMS.Controllers.TinhLuong.Fillet
{
    [Authorize]
    public class ChinhFilletController : Controller
    {
        private readonly IHubContext<PMS.Hubs.ProgressHub> _hubContext;
        private readonly IHttpClientFactory _httpClientFactory;

        public ChinhFilletController(IHubContext<PMS.Hubs.ProgressHub> hubContext, IHttpClientFactory httpClientFactory)
        {
        ...
        public IActionResult ChinhView()
        {
            ...
            ViewBag.titile = "Tính Lương Fillet Chính";
            return View("~/Views/TinhLuong/Fillet/ChinhView.cshtml");
        }
        public async Task<IEnumerable<object>> LoadTongHopTinhLuongFillet(DateTime dateTime, string xuongId)
        {
            IEnumerable<object> dataSource = ViewBag.dataSource;
            if (dataSource == null)
            {
                var apiUrl = $"{AppViewModels.AppViewModel.Instance.ApiHostUrl}/api/PhieuCanTPFillets/LoadTongHopTinhLuong/{dateTime.ToString("yyyy-MM-dd")}/{xuongId}";
        ...
        public async Task<ActionResult> Reload(DateTime dateTime, string xuongId, string typeReload)
        {
            ...
            try
            {
                switch (typeReload)
                {
                    case "LoadTongHopTinhLuongFillet":
                        dataSource = await LoadTongHopTinhLuongFillet(dateTime, xuongId);
                        break;
        ...
        public async Task<IActionResult> KetChuyen(DateTime dateTime, string maXuong, string listSanLuongPhuFillet)
        {
            try
            {
                var apiUrl = $"{AppViewModels.AppViewModel.Instance.ApiHostUrl}/api/PhieuCanTPFillets/KetChuyenTinhLuong/{dateTime.ToString("yyyy-MM-dd")}/{maXuong}";
        ...
        private async Task CallExecuteTPSoftActionApi(DateTime dateTime, string maXuong)
        {
            try
            {
                var apiUrl = $"{AppViewModels.AppViewModel.Instance.ApiHostUrl}/api/PhieuCanTPFillets/TPSoftAction_Fillet/{dateTime.ToString("yyyy-MM-dd")}/{maXuong}";
