...
{
    [Authorize]
    public class HQ_HuyThucDonController : Controller
    {
        private readonly IHttpClientFactory _httpClientFactory;
        public HQ_HuyThucDonController(IHttpClientFactory httpClientFactory)
        {
        ...
        public IActionResult Index()
        {
            ...
            var loaiMonAns = helperLoaiMonAn.GetAsync<IEnumerable<HQ_HuyThucDon>>(HttpContext, apiLoaiMonAnUrl);
            ViewBag.loaiMonAns = loaiMonAns.Result.ToList();

            ViewBag.TitlePage = "Món Ăn";
            return View("~/Views/NhaAn/DanhMucNhaAn/HQ_HuyThucDon/MonAnView.cshtml");
        }
        public async Task<IEnumerable<HQ_HuyThucDon>> GetAlls()
        {
            IEnumerable<HQ_HuyThucDon> dataSource = ViewBag.dataSource;
            if (dataSource == null)
            {
                var apiUrl = $"{AppViewModels.AppViewModel.Instance.ApiHostUrl}/api/HQ_HuyThucDons/GetAlls";
                using var helper = new Middlewares.MethodRESTFulAPIHelpers(_httpClientFactory);
                ViewBag.dataSource = await helper.GetAsync<IEnumerable<HQ_HuyThucDon>>(HttpContext, apiUrl);
        ...
        public async Task<IEnumerable<object>> GetAllsFullField()
        {
            ...
            if (dataSource == null)
            {
                var apiUrl = $"{AppViewModels.AppViewModel.Instance.ApiHostUrl}/api/HQ_HuyThucDons/GetAllsFullField";
                using var helper = new Middlewares.MethodRESTFulAPIHelpers(_httpClientFactory);
        ...
        {
            var apiUrl = $"{AppViewModels.AppViewModel.Instance.ApiHostUrl}/api/HQ_HuyThucDons/Insert";
            try
            {
        ...
        public async Task<IActionResult> GetsByMa(int id)
        {
            ...
            if (dataSource == null)
            {
                var apiUrl = $"{AppViewModels.AppViewModel.Instance.ApiHostUrl}/api/HQ_HuyThucDons/GetsByMa/{id}";
                using var helper = new Middlewares.MethodRESTFulAPIHelpers(_httpClientFactory);
                var item = await helper.GetAsync<HQ_HuyThucDon>(HttpContext, apiUrl);
                if (item != null)
                {
                    return Json(new
                    {
        ...
        //{
        //    var apiUrl = $"{AppViewModels.AppViewModel.Instance.ApiHostUrl}/api/HQ_HuyThucDons/Update/{id}";
        //    try
        //    {
        ...
        public async Task<IActionResult> DoDelete(int id)
        {
            try
            {
                var apiUrl = $"{AppViewModels.AppViewModel.Instance.ApiHostUrl}/api/HQ_HuyThucDons/Delete/{id}";
                using var helper = new Middlewares.MethodRESTFulAPIHelpers(_httpClientFactory);
