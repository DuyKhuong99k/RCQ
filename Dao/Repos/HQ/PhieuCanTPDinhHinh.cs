...
order by
    ...
    p.Gio";
        ...
        {
            connection.Open();
            var items = connection.QueryAsync<T>(query, new { fromDate = fromDate.Date,toDate=toDate.Date, xuongId,maNhanVien , isDinhMucBinhThuong}).Result.ToList();
            return items;
        }
    }
...
order by
    ...
    p.Gio";
        ...
        {
            connection.Open();
            var items = connection.QueryAsync<T>(query, new { fromDate = fromDate.Date,toDate=toDate.Date, xuongId,maHoSo ,isDinhMucBinhThuong}).Result.ToList();
            return items;
        }
    }
...
order by
    ...
    p.Gio";
        ...
        {
            connection.Open();
            var items = connection.QueryAsync<T>(query, new { fromDate = fromDate.Date,toDate=toDate.Date, xuongId,maThe,isDinhMucBinhThuong }).Result.ToList();
            return items;
        }
    }
...
where
    ...
    {
        var query =
            @"Select IsNull( Count(*),0) as Item1,isNull( Sum(TrongLuongTra),0) As Item2 from PhieuCanTPDinhHinh where MaNhanVien = @nhanVienId and Ngay =@ngay";
        using (var connection = new SqlConnection(connectionString))
        {
            connection.Open();
            var row = connection.Query<Tuple<int, decimal>>(
...
group by
	...
	p.MaXuong";
        using (var connection = new SqlConnection(connectionString))
        {
            ...
            var items = connection.QueryAsync<T>(
                    query,
                    new { fromDate = fromDate.Date, toDate = toDate.Date, xuongId, isDinhMucBinhThuong, isCaTraChuyenDoi, isFloor })
                .Result
                .ToList();
            return items;
        }
