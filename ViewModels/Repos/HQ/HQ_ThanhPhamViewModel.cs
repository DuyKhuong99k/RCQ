...
public partial class HQ_ThanhPhamViewModel : ObservableObject
{
    ...
    private HQ_ThanhPham? selectedItem;

    [ObservableProperty] private ObservableRangeCollection<object> selectedItems = new();
    [ObservableProperty] private ObservableRangeCollection<HQ_ThanhPham> usedItems = new();
    ...
    public List<HQ_ThanhPham_D> GetDs(string Ngay, int PageIndex, int PageSize)
    {
        ...
        return db.HqThanhPhamDs.Where(x => x.MNgay.Date >= date.Date).OrderByDescending(x => x.MNgay)
    ...
    public List<HQ_ThanhPham_U> GetUs(string Ngay, int PageIndex, int PageSize)
    {
        ...
        var date = new DateTime();
        date = DateTime.ParseExact(Ngay, "yyyyMMddHHmmss", CultureInfo.InvariantCulture);
        var latestDates = db.HqThanhPhamUs
            .Where(x => x.MNgay.Date >= date.Date)
            .GroupBy(x => x.ThanhPhamId)
            .Select(g => new { ThanhPhamId = g.Key, MaxId = g.Max(x => x.Id) });

        var query = from hq in db.HqThanhPhamUs.Where(x => x.MNgay.Date >= date.Date) 
                    join latest in latestDates
                    on new { hq.ThanhPhamId, hq.Id } 
                    equals new { latest.ThanhPhamId, Id = latest.MaxId }
        ...
        if (items.Any())
            lock (Items)
            {
                try
                {
                    ...
                    {
                        Items.Add(item);
                        if (item.SuDung) UsedItems.Add(item);
                    }
                }
