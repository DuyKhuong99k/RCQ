...
namespace WebAPI.Controllers
{
    ...
    [ApiController]
    public class PhieuCanTaiChesController : ControllerBase
    {
        private readonly dbPMScontext _context;

        public PhieuCanTaiChesController(dbPMScontext context)
        {
            _context = context;
        }
        ...
        public async Task<ActionResult<IEnumerable<object>>> GetPhieuCan_XLPC(string dateTime, string xuongId)
        {
            ...
            var items = Vm.VmPhieuCanTaiChe.GetPhieuCan_XLPC<object>(date1, xuongId);
            return items;
        }
        [HttpGet("{dateTime}/{xuongId}")]
        [Authorize]
        ...
        {
            DateTime date1 = DateTime.ParseExact(dateTime, "yyyy-MM-dd", System.Globalization.CultureInfo.InvariantCulture);
            var items = _context.PhieuCanTaiChe.Where(x => x.Ngay == date1 && x.MaXuong == xuongId).OrderByDescending(x => x.Ngay).ToList();
        ...
        {
            var model = JsonSerializer.Deserialize<PhieuCanTaiChe>(dataT.Item1);
            ...
            if (_context.PhieuCanTaiChe.Any(u => u.STT == model.STT && u.Ngay == model.Ngay && u.MaXuong == model.MaXuong))
            {
                return BadRequest(new ApiResponse
                {
                    Success = false,
            ...
            var newItem = new PhieuCanTaiChe
            {
                STT = model.STT,
                Ngay = model.Ngay,
            ...
            _context.PhieuCanTaiChe.Add(newItem);
            try
            {
        ...
        public IActionResult GetsByMa(int stt, string ngay, string maMayCan, string maXuong)
        {
            ...
            var item = _context.PhieuCanTaiChe.FirstOrDefault(x => x.STT == stt && x.Ngay == ngayConvert && x.MaMayCan == maMayCan && x.MaXuong == maXuong);
            if (item == null)
            {
        ...
        public async Task<IActionResult> Update(int stt, string ngay, string maMayCan, string maXuong, Tuple<string> dataT)
        {
            ...
            DateTime ngayConvert = DateTime.ParseExact(ngay, "yyyy-MM-dd", System.Globalization.CultureInfo.InvariantCulture);
            var item = await _context.PhieuCanTaiChe.FirstOrDefaultAsync(x => x.STT == stt && x.Ngay == ngayConvert && x.MaMayCan == maMayCan && x.MaXuong == maXuong);
        ...
        public async Task<IActionResult> Delete(int stt, string ngay, string maMayCan, string maXuong, Tuple<string> dataT)
        {
            ...
            using (var transaction = await _context.Database.BeginTransactionAsync())
            {
                try
                {
                    _context.PhieuCanTaiChe.Remove(item);
                    _context.PhieuCanTaiChe.Add(newItem);
                    await _context.SaveChangesAsync();
        ...
        [HttpGet("{fromDate}/{toDate}/{xuongId}")]
        [Authorize]
        public async Task<ActionResult<IEnumerable<object>>> GetPhieuCanRaDongXepKhuonsByDateToDate(string fromDate, string toDate, string xuongId)
        {
            if (_context.PhieuCanTaiChe == null)
            {
                return NotFound();
            ...
            var items = Vm.VmPhieuCanTaiChe.GetPhieuCanRaDongXepKhuonsByDateToDate<object>(date1, date2, xuongId);
            return items;
        }
        [HttpGet("{fromDate}/{toDate}/{xuongId}")]
        [Authorize]
        public async Task<ActionResult<IEnumerable<object>>> GetPhieuCanTaiChes(string fromDate, string toDate, string xuongId)
        {
            if (_context.PhieuCanTaiChe == null)
            {
                return NotFound();
            ...
            DateTime date1 = DateTime.ParseExact(fromDate, "yyyy-MM-dd", CultureInfo.InvariantCulture);
            DateTime date2 = DateTime.ParseExact(toDate, "yyyy-MM-dd", CultureInfo.InvariantCulture);
            var items = Vm.VmPhieuCanTaiChe.GetPhieuCanTaiChes<object>(date1, date2, xuongId);
            return items;
        }
        [HttpGet("{fromDate}/{toDate}/{xuongId}")]
        [Authorize]
        public async Task<ActionResult<IEnumerable<object>>> GetPhieuCanTongHopTaiChe(string fromDate, string toDate, string xuongId)
        {
            if (_context.PhieuCanTaiChe == null)
            {
                return NotFound();
            ...
            DateTime date1 = DateTime.ParseExact(fromDate, "yyyy-MM-dd", CultureInfo.InvariantCulture);
            DateTime date2 = DateTime.ParseExact(toDate, "yyyy-MM-dd", CultureInfo.InvariantCulture);
            var items = Vm.VmPhieuCanTaiChe.GetPhieuCanTongHopTaiChe<object>(date1, date2, xuongId);
            return items;
        }
        #endregion
    }
