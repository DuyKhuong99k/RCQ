...
namespace WebAPI.Controllers
{
    ...
    [ApiController]
    public class BV_PhieuCanBotCaController : ControllerBase
    {
        private readonly dbPMScontext _context;

        public BV_PhieuCanBotCaController(dbPMScontext context)
        {
            _context = context;
        }
        ...
        public IActionResult CheckItemServer(string ngay)
        {
            ...
            try
            {
                var vmBV_PhieuCanBot = Vm.VmBV_PhieuCanBotCa;
                //lấy danh sách item từ server
                var itemsServer = vmBV_PhieuCanBot.Gets<object>(dateTime);
        ...
        public IActionResult DeleteItemServer(string ngay)
        {
            ...
            {
                var vmBV_PhieuCanBot = Vm.VmBV_PhieuCanBotCa;
                vmBV_PhieuCanBot.Delete(dateTime);
                return Ok(new ApiResponse
                {
                    Success = true,
        ...
        public IActionResult ConfirmItemCountUpload(string ngay)
        {
            ...
            try
            {
                var vmPhieuCan = Vm.VmBT_PhieuCan;
                ...
                else
                {
                    ...
                    {
                        Success = false,
                        Message = $@"Không có dữ liệu Phiếu Cân Bột Cá"
                    });
                }
            }
            catch (Exception ex)
            ...
            try
            {
                ...
                if (items != null && items.Any())
                {
                    var _items = (from p in items
                                  select new BravoModelV1.Model.PhieuCanBotCa
                                  {
                                      ThoiGian = TimeSpan.Parse(p.Gio.ToString()),
                                      SoLuong = Convert.ToDecimal(p.TrongLuong),
                                      ID =
                                $@"{vmApp.DateTimeNow:yyyyMMdd}.{vmApp.PCName}.{AppViewModels.HardId.GetValue()}.{p.Ngay:yyyyMMdd}.{p.MaXuong}.{p.MaMayCan}.{p.STT}"
                                    .NonUnicode().Replace(" ", "").ToUpper(),
                                      MaKhachHang = $@"{p.KhachHangName}".NonUnicode(true).Replace(" ", "").ToUpper(),
                                      MaSanPham = $@"{p.ThanhPhamName}".NonUnicode(true).Replace(" ", "").ToUpper(),
                                      Ngay = Convert.ToDateTime(p.Ngay),
                                      MaPhuongTien = "",
                                      TenPhuongTien = "",
                                      TenKhachHang = $@"{p.KhachHangName}",
                                      TenSanPham = $@"{p.ThanhPhamName}",
                                  }).ToList();
                    if (_items.Any())
                    {
                        var rows = vmBV_PhieuCanBot.InsertBatch(_items);
                        return Ok(new ApiResponse
                        {
                            Success = true,
                    ...
                    else
                    {
                        ...
                        {
                            Success = false,
                            Message = "Không thể tổng hợp được dữ liệu Phiếu Cân Bột Cá.",
                        });
                    }
                }
                else
