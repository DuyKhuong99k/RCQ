...
namespace WebAPI.Controllers
{
    ...
    [ApiController]
    public class PD_PhieuCanController : ControllerBase
    {
        private readonly dbPMScontext _context;

        public PD_PhieuCanController(dbPMScontext context)
        {
            _context = context;
        }
        ...
        public async Task<ActionResult<IEnumerable<object>>> GetPhieuCan_XLPC(string dateTime, string xuongId)
        {
            ...
            var items = Vm.VmPD_PhieuCan.GetPhieuCan_XLPC<object>(date1, xuongId);
            return items;
        }
        [HttpGet("{dateTime}/{xuongId}")]
        [Authorize]
        ...
        {
            DateTime date1 = DateTime.ParseExact(dateTime, "yyyy-MM-dd", System.Globalization.CultureInfo.InvariantCulture);
            var items = _context.PD_PhieuCan.Where(x => x.Ngay == date1 && x.MaXuong == xuongId).OrderByDescending(x => x.Ngay).ToList();
        ...
        {
            var model = JsonSerializer.Deserialize<PD_PhieuCan>(dataT.Item1);
            ...
            if (_context.PD_PhieuCan.Any(u => u.STT == model.STT && u.Ngay == model.Ngay && u.MaXuong == model.MaXuong))
            {
                return BadRequest(new ApiResponse
                {
                    Success = false,
            ...
            _context.PD_PhieuCan.Add(newItem);
            try
            {
        ...
        public IActionResult GetsByMa(int stt, string ngay, string maMayCan, string maXuong)
        {
            ...
            var item = _context.PD_PhieuCan.FirstOrDefault(x => x.STT == stt && x.Ngay == ngayConvert && x.MaMayCan == maMayCan && x.MaXuong == maXuong);
            if (item == null)
            {
        ...
        public async Task<IActionResult> Update(int stt, string ngay, string maMayCan, string maXuong, Tuple<string> dataT)
        {
            ...
            DateTime ngayConvert = DateTime.ParseExact(ngay, "yyyy-MM-dd", System.Globalization.CultureInfo.InvariantCulture);
            var item = await _context.PD_PhieuCan.FirstOrDefaultAsync(x => x.STT == stt && x.Ngay == ngayConvert && x.MaMayCan == maMayCan && x.MaXuong == maXuong);
            ...
            item.MaCongThuc = model.MaCongThuc;
            item.MaLyDo = model.MaLyDo;
        ...
        public async Task<IActionResult> Delete(int stt, string ngay, string maMayCan, string maXuong, Tuple<string> dataT)
        {
            ...
            using (var transaction = await _context.Database.BeginTransactionAsync())
            {
                try
                {
                    _context.PD_PhieuCan.Remove(item);
                    _context.PD_PhieuCan.Add(newItem);
                    await _context.SaveChangesAsync();
        ...
        public async Task<ActionResult<IEnumerable<object>>> GetsChiTiet(string fromDate, string toDate, string xuongId)
        {
            ...
            var items = Vm.VmPD_PhieuCan.GetsChiTiet<object>(date1, date2, xuongId);
            return items;
        }
        [HttpGet("{fromDate}/{toDate}/{xuongId}")]
        [Authorize]
        public async Task<ActionResult<IEnumerable<object>>> GetsTongHop(string fromDate, string toDate, string xuongId)
        {
            if (_context.PhieuCanTaiChe == null)
            {
                return NotFound();
            ...
            DateTime date1 = DateTime.ParseExact(fromDate, "yyyy-MM-dd", CultureInfo.InvariantCulture);
            DateTime date2 = DateTime.ParseExact(toDate, "yyyy-MM-dd", CultureInfo.InvariantCulture);
            var items = Vm.VmPD_PhieuCan.GetsTongHop<object>(date1, date2, xuongId);
            return items;
        }
        #endregion
    }
