...
namespace WebAPI.Controllers
{
    ...
    [ApiController]
    public class PhieuCanXepKhuonBlocksController : ControllerBase
    {
        private readonly dbPMScontext _context;

        public PhieuCanXepKhuonBlocksController(dbPMScontext context)
        {
            _context = context;
        }
        ...
        {
            if (_context.PhieuCanXepKhuonBlock == null)
            {
                return NotFound();
            }
            var items = Vm.VmPhieuCanXepKhuonBlock.GetTongHopThanhPhams<object>(fromDate, toDate, xuongId);
            return items;
        }
        [HttpGet("{fromDate},{toDate},{xuongId}")]
        [Authorize]
        public async Task<ActionResult<IEnumerable<object>>> GetTongHopThanhPhamsDB(DateTime fromDate, DateTime toDate, string xuongId)
        {
            ...
            var items = Vm.VmDashBoard.ItemTongHopThanhPhamBlockXepKhuon.OfType<dynamic>().Where(x => x.MaXuong == xuongId).ToList();// Sử dụng dynamic để cast các object
            return items;
        }
        ...
        public async Task<ActionResult<IEnumerable<object>>> GetPhieuCan_XLPC(string dateTime, string xuongId)
        {
            ...
            var items = Vm.VmPhieuCanXepKhuonBlock.GetPhieuCan_XLPC<object>(date1, xuongId);
            return items;
        }
        [HttpGet("{dateTime}/{xuongId}")]
        [Authorize]
        ...
        {
            DateTime date1 = DateTime.ParseExact(dateTime, "yyyy-MM-dd", System.Globalization.CultureInfo.InvariantCulture);
            var items = _context.PhieuCanXepKhuonBlock.Where(x => x.Ngay == date1 && x.MaXuong == xuongId).OrderByDescending(x => x.Ngay).ToList();
        ...
        {
            var model = JsonSerializer.Deserialize<PhieuCanXepKhuonBlock>(dataT.Item1);
            ...
            if (_context.PhieuCanXepKhuonBlock.Any(u => u.STT == model.STT && u.Ngay == model.Ngay && u.MaXuong == model.MaXuong))
            {
                return BadRequest(new ApiResponse
                {
                    Success = false,
            ...
            var newItem = new PhieuCanXepKhuonBlock
            {
                STT = model.STT,
                Ngay = model.Ngay,
            ...
            _context.PhieuCanXepKhuonBlock.Add(newItem);
            try
            {
        ...
        public IActionResult GetsByMa(int stt, string ngay, string maMayCan, string maXuong)
        {
            ...
            var item = _context.PhieuCanXepKhuonBlock.FirstOrDefault(x => x.STT == stt && x.Ngay == ngayConvert && x.MaMayCan == maMayCan && x.MaXuong == maXuong);
            if (item == null)
            {
        ...
        public async Task<IActionResult> Update(int stt, string ngay, string maMayCan, string maXuong, Tuple<string> dataT)
        {
            ...
            DateTime ngayConvert = DateTime.ParseExact(ngay, "yyyy-MM-dd", System.Globalization.CultureInfo.InvariantCulture);
            var item = await _context.PhieuCanXepKhuonBlock.FirstOrDefaultAsync(x => x.STT == stt && x.Ngay == ngayConvert && x.MaMayCan == maMayCan && x.MaXuong == maXuong);
        ...
        public async Task<IActionResult> Delete(int stt, string ngay, string maMayCan, string maXuong, Tuple<string> dataT)
        {
            ...
            using (var transaction = await _context.Database.BeginTransactionAsync())
            {
                try
                {
                    _context.PhieuCanXepKhuonBlock.Remove(item);
                    _context.PhieuCanXepKhuonBlock.Add(newItem);
                    await _context.SaveChangesAsync();
        ...
        [HttpGet("{fromDate}/{toDate}/{xuongId}")]
        [Authorize]
        public async Task<ActionResult<IEnumerable<object>>> GetPhieuCanBlocks(string fromDate, string toDate, string xuongId)
        {
            if (_context.PhieuCanXepKhuonBlock == null)
            {
                return NotFound();
            ...
            var items = Vm.VmPhieuCanXepKhuonBlock.GetPhieuCanBlocks<object>(date1, date2, xuongId);
            return items;
        }
        [HttpGet("{fromDate}/{toDate}/{xuongId}")]
        [Authorize]
        public async Task<ActionResult<IEnumerable<object>>> GetPhieuCanTongHopBlock(string fromDate, string toDate, string xuongId)
        {
            if (_context.PhieuCanXepKhuonBlock == null)
            {
                return NotFound();
            ...
            DateTime date1 = DateTime.ParseExact(fromDate, "yyyy-MM-dd", CultureInfo.InvariantCulture);
            DateTime date2 = DateTime.ParseExact(toDate, "yyyy-MM-dd", CultureInfo.InvariantCulture);
            var items = Vm.VmPhieuCanXepKhuonBlock.GetPhieuCanTongHopBlock<object>(date1, date2, xuongId);
            return items;
        }
        #endregion
    }
