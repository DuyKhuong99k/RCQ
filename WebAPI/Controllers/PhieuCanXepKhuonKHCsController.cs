...
namespace WebAPI.Controllers
{
    ...
    [ApiController]
    public class PhieuCanXepKhuonKHCsController : ControllerBase
    {
        private readonly dbPMScontext _context;

        public PhieuCanXepKhuonKHCsController(dbPMScontext context)
        {
            _context = context;
        }
        ...
        {
            if (_context.PhieuCanXepKhuonKHC == null)
            {
                return NotFound();
            }
            var items = Vm.VmPhieuCanXepKhuonKHC.GetTongHopThanhPhams<object>(fromDate, toDate, xuongId);
            return items;
        }
        [HttpGet("{fromDate},{toDate},{xuongId}")]
        [Authorize]
        public async Task<ActionResult<IEnumerable<object>>> GetTongHopThanhPhamsDB_St(DateTime fromDate, DateTime toDate, string xuongId)
        {
            ...
            var items = Vm.VmDashBoard.ItemTongHopThanhPhamKHCXepKhuon.OfType<dynamic>().Where(x => x.MaXuong == xuongId).ToList();// Sử dụng dynamic để cast các object
            return items;
        }
        [HttpGet("{fromDate},{toDate},{xuongId}")]
        [Authorize]
        ...
        public async Task<ActionResult<IEnumerable<object>>> GetPhieuCan_XLPC(string dateTime, string xuongId)
        {
            ...
            var items = Vm.VmPhieuCanXepKhuonKHC.GetPhieuCan_XLPC<object>(date1, xuongId);
            return items;
        }
        [HttpGet("{dateTime}/{xuongId}")]
        [Authorize]
        ...
        {
            DateTime date1 = DateTime.ParseExact(dateTime, "yyyy-MM-dd", System.Globalization.CultureInfo.InvariantCulture);
            var items = _context.PhieuCanXepKhuonKHC.Where(x => x.Ngay == date1 && x.MaXuong == xuongId).OrderByDescending(x => x.Ngay).ToList();
        ...
        {
            var model = JsonSerializer.Deserialize<PhieuCanXepKhuonKHC>(dataT.Item1);
            ...
            if (_context.PhieuCanXepKhuonKHC.Any(u => u.STT == model.STT && u.Ngay == model.Ngay && u.MaXuong == model.MaXuong))
            {
                return BadRequest(new ApiResponse
                {
                    Success = false,
            ...
            var newItem = new PhieuCanXepKhuonKHC
            {
                STT = model.STT,
                Ngay = model.Ngay,
                Gio = model.Gio,
                ...
                DaXacNhan = model.DaXacNhan,
                MaNhanVien = model.MaNhanVien,
                Block = model.Block
            };
            _context.PhieuCanXepKhuonKHC.Add(newItem);
            try
            {
        ...
        public IActionResult GetsByMa(int stt, string ngay, string maMayCan, string maXuong)
        {
            ...
            var item = _context.PhieuCanXepKhuonKHC.FirstOrDefault(x => x.STT == stt && x.Ngay == ngayConvert && x.MaMayCan == maMayCan && x.MaXuong == maXuong);
            if (item == null)
            {
        ...
        public async Task<IActionResult> Update(int stt, string ngay, string maMayCan, string maXuong, Tuple<string> dataT)
        {
            ...
            DateTime ngayConvert = DateTime.ParseExact(ngay, "yyyy-MM-dd", System.Globalization.CultureInfo.InvariantCulture);
            var item = await _context.PhieuCanXepKhuonKHC.FirstOrDefaultAsync(x => x.STT == stt && x.Ngay == ngayConvert && x.MaMayCan == maMayCan && x.MaXuong == maXuong);
        ...
        public async Task<IActionResult> Delete(int stt, string ngay, string maMayCan, string maXuong, Tuple<string> dataT)
        {
            ...
            using (var transaction = await _context.Database.BeginTransactionAsync())
            {
                try
                {
                    _context.PhieuCanXepKhuonKHC.Remove(item);
                    _context.PhieuCanXepKhuonKHC.Add(newItem);
                    await _context.SaveChangesAsync();
        ...
        [HttpGet("{fromDate}/{toDate}/{xuongId}")]
        [Authorize]
        public async Task<ActionResult<IEnumerable<object>>> GetPhieuCanXepKhuonKHCs(string fromDate, string toDate, string xuongId)
        {
            if (_context.PhieuCanXepKhuonKHC == null)
            {
                return NotFound();
            ...
            var items = Vm.VmPhieuCanXepKhuonKHC.GetPhieuCanXepKhuonKHCs<object>(date1, date2, xuongId);
            return items;
        }
        [HttpGet("{fromDate}/{toDate}/{xuongId}")]
        [Authorize]
        public async Task<ActionResult<IEnumerable<object>>> GetPhieuCanTongHopKHCs(string fromDate, string toDate, string xuongId)
        {
            if (_context.PhieuCanXepKhuonKHC == null)
            {
                return NotFound();
            ...
            DateTime date1 = DateTime.ParseExact(fromDate, "yyyy-MM-dd", CultureInfo.InvariantCulture);
            DateTime date2 = DateTime.ParseExact(toDate, "yyyy-MM-dd", CultureInfo.InvariantCulture);
            var items = Vm.VmPhieuCanXepKhuonKHC.GetPhieuCanTongHopKHCs<object>(date1, date2, xuongId);
            return items;
        }
        [HttpGet("{fromDate}/{toDate}/{xuongId}")]
        [Authorize]
        public async Task<ActionResult<IEnumerable<object>>> GetPhieuCanTongHopKHCsTheoNhanVien(string fromDate, string toDate, string xuongId)
        {
            if (_context.PhieuCanXepKhuonKHC == null)
            {
                return NotFound();
            ...
            DateTime date1 = DateTime.ParseExact(fromDate, "yyyy-MM-dd", CultureInfo.InvariantCulture);
            DateTime date2 = DateTime.ParseExact(toDate, "yyyy-MM-dd", CultureInfo.InvariantCulture);
            var items = Vm.VmPhieuCanXepKhuonKHC.GetPhieuCanTongHopKHCsTheoNhanVien<object>(date1, date2, xuongId);
            return items;
        }
        #endregion
