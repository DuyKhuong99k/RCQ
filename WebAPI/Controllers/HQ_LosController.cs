...
namespace WebAPI.Controllers
{
    ...
    [ApiController]
    public class HQ_LosController : ControllerBase
    {
        private readonly dbPMScontext _context;

        public HQ_LosController(dbPMScontext context)
        {
            _context = context;
        }
        ...
        {
            var items = _context.HqLos.OrderByDescending(x => x.Id).ToList();
        ...
        {
            var item = _context.HqLos.FirstOrDefault(x => x.Id == id);
            if (item == null)
            {
        ...
        [HttpPost]
        [Authorize]
        public async Task<IActionResult> Insert(HQ_Lo model)
        {
            ...
            if (_context.HqLos.Any(u => u.Id == model.Id))
            {
                return BadRequest(new ApiResponse
                {
                    Success = false,
        ...
        [HttpPost("{id}")]
        [Authorize]
        public async Task<IActionResult> Update(string id, [FromBody] HQ_Lo model)
        {
            ...
            var item = await _context.HqLos.FindAsync(id);
            if (item == null)
            {
                return BadRequest(new ApiResponse
                {
