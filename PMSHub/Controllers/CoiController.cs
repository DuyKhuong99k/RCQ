...
namespace PMSHub.Controllers
{
    ...
    [ApiController]
    public class CoiController(IMainService mainService, IMayCansService mayCansService) : ControllerBase
    {
        [HttpGet]
        public IActionResult Gets(string Ngay, int PageIndex, int PageSize)
        {
            ...
            if (index == -1)
            {
                ...
                var items = mainService.VmCoi.Items.Where(x => x.MNgay.Date >= date.Date).Skip((PageIndex - 1) * PageSize).Take(PageSize).Select(x => new { Id = x.Ma, Ten = x.Ten, SuDung = true, MNgay = $"{x.MNgay.ToString("yyyyMMddHHmmss")}" }).ToList();
                var json = new
                {
                    data = items,
                    total = items.Count,
        ...
        public IActionResult GetUs(string Ngay, int PageIndex, int PageSize)
        {
            ...
            if (index == -1)
            {
                var items = mainService.VmCoi.GetUs(Ngay, PageIndex, PageSize);
                var json = new
                {
                    data = items.Select(x => new { Id = x.MaCoi, Ten = x.Ten, SuDung = true, MNgay = $"{x.MNgay.ToString("yyyyMMddHHmmss")}" }).ToList(),
                    total = items.Count,
                    PageIndex
                };
            ...
            else
            {
                ...
                if (mayCan != null)
                {
                    var items = mainService.VmCoi.GetUs(mNgay, PageIndex, PageSize);
                    var json = new
                    {
                        data = items.Select(x => new { Id = x.MaCoi, Ten = x.Ten, SuDung = true, MNgay = $"{x.MNgay.ToString("yyyyMMddHHmmss")}" }).ToList(),
                        total = items.Count,
        ...
        public IActionResult GetDs(string Ngay, int PageIndex, int PageSize)
        {
            ...
            if (index == -1)
            {
                var items = mainService.VmCoi.GetDs(Ngay, PageIndex, PageSize);
                var json = new
                {
                    data = items.Select(x => new { Id = x.MaCoi, Ten = x.Ten, SuDung = true, MNgay = $"{x.MNgay.ToString("yyyyMMddHHmmss")}" }).ToList(),
                    total = items.Count,
            ...
            else
            {
                ...
                if (mayCan != null)
                {
                    var items = mainService.VmCoi.GetDs(mNgay, PageIndex, PageSize);
                    var json = new
                    {
                        data = items.Select(x => new { Id = x.MaCoi, Ten = x.Ten, SuDung = true, MNgay = $"{x.MNgay.ToString("yyyyMMddHHmmss")}" }).ToList(),
                        total = items.Count,
