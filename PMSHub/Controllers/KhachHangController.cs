...
public class KhachHangController(IMainService mainService, IMayCansService mayCansService) : ControllerBase
{
    [HttpGet]
    public IActionResult Gets(string Ngay, int PageIndex, int PageSize)
    {
        ...
        if (index == -1)
        {
            ...
            var items = mainService.VmKhachHangXepKhuon.Items.Where(x => x.SuDung == true)
            ...
            if (mayCan != null)
            {
                switch (mayCan.WKv)
                {
                    ...
                    case AppKV.PhuPhamv2:
                        break;
                    case AppKV.Hq:
                        var itemshq = mainService.VmKhachHangXepKhuon.Items.Where(x => x.SuDung == true)
                            .Skip((PageIndex - 1) * PageSize).Take(PageSize).Select(x => new
                            {
                                Id = x.Ma, Ten = x.Ten, x.SuDung, MNgay = $"{DateTime.Now.ToString("yyyyMMddHHmmss")}"
                            }).ToList();
