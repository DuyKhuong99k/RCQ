...
namespace PMSHub.Controllers
{
    ...
    [ApiController]
    public class ThanhPhamController(IMainService mainService) : ControllerBase
    {
        [HttpGet]
        public IActionResult Gets(string Ngay,int PageIndex,int PageSize)
        {
         
            var items = mainService.VmThanhPhamHq.Items.Skip((PageIndex -1)* PageSize).Take(PageSize).Select(x=>new {Id=x.Id,Ten=x.Ten,x.SuDung,MNgay =$"{x.MNgay.ToString("yyyyMMddHHmmss")}"}).ToList();
            var json = new
            {
                data = items,
                total = items.Count,
