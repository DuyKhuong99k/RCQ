...
public class NETController(IMainService mainService, IMayCansService mayCansService) : ControllerBase
{
    [HttpGet]
    public IActionResult Gets(string Ngay, int PageIndex, int PageSize)
    {
        ...
        if (index == -1)
        {
            ...
            var items = mainService.VmNETXepKhuon.Items.Where(x => x.SuDung == true)
        ...
        else
        {
            ...
            if (mayCan != null)
            {
                switch (mayCan.WKv)
                {
                    ...
                    case AppKV.Hq:
                    {
                        var itemshq = mainService.VmNETXepKhuon.Items.Where(x => x.SuDung == true)
                            .Skip((PageIndex - 1) * PageSize).Take(PageSize).Select(x => new
                            {
                                Id = x.Ma, Ten = x.Ten, x.SuDung, MNgay = $"{DateTime.Now.ToString("yyyyMMddHHmmss")}"
                            }).ToList();
